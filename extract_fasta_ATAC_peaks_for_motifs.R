...
atac_sub_clust <- read.xlsx('../Output_YR/toxo_cdc/ME49_59/tables/atac_clusters_within_rna_tran_dtw_clusters_manual/rna_atac_sub_clusters.xlsx')
atac_sub_clust$GeneID <- gsub('-', '_', atac_sub_clust$GeneID)
peak.genes.bed.merged.bed <- read.xlsx("../Input_YR//toxo_scATAC_MJ_ME49_59/peak_gene_assigned_final.xlsx")
atac_sub_clust <- inner_join(atac_sub_clust, peak.genes.bed.merged.bed, by = c('GeneID' = 'gene_name'))

unique.atac.sub.clusts <- unique(atac_sub_clust$trans.rna.atac.clust)
num.subclusts <- length(unique.atac.sub.clusts)
bed.dir <- '../Output_YR/toxo_cdc/ME49_59/tables/atac_clusters_within_rna_tran_dtw_clusters_manual/bed/'
fasta.dir <- '../Output_YR/toxo_cdc/ME49_59/tables/atac_clusters_within_rna_tran_dtw_clusters_manual/fasta/'
lapply(1:num.subclusts, function(i){
  sc <- unique.atac.sub.clusts[i]
  tmp <- atac_sub_clust %>% dplyr::filter(trans.rna.atac.clust == sc)
  tmp <- tmp[,4:9]
  out.bed <- paste(bed.dir, sc, '.bed', sep = "")
  write.table(tmp,out.bed,
              sep = "\t", quote = F, row.names = F, col.names = F)
  out.fasta <- paste(fasta.dir, sc, '.fasta', sep = "")
  bedtoolsr::bt.getfasta(fi = '../Input_YR/Toxo_genomics/genome/ToxoDB-59_TgondiiME49_Genome.fasta', bed = out.bed, fo = out.fasta, s = T)
})
...
peak.genes <- read.xlsx("../Input_YR//toxo_scATAC_MJ_ME49_59/scATAC_peaks_genes_assigned.xlsx")
